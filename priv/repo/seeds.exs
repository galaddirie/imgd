...
defmodule SeedHelpers do
  def node_id(prefix), do: "#{prefix}_#{:crypto.strong_rand_bytes(4) |> Base.encode16(case: :lower)}"
...
{:ok, _wf_api_fetch} = Workflows.create_workflow(scope, %{
...
{:ok, wf_transform} = Workflows.create_workflow(scope, %{
  ...
  nodes: [
    ...
    %{
      ...
      config: %{
        "template" => "Hello {{name}}! Your email is {{email}} and you work at {{company.name}}."
      },
  ...
  connections: [
    %{id: SeedHelpers.conn_id(), source_node_id: transform_http, target_node_id: transform_pick},
    %{id: SeedHelpers.conn_id(), source_node_id: transform_pick, target_node_id: transform_format},
    %{id: SeedHelpers.conn_id(), source_node_id: transform_format, target_node_id: transform_debug}
  ],
  triggers: [%{type: :manual, config: %{}}],
...
{:ok, _} = Workflows.publish_workflow(scope, wf_transform, %{
  version_tag: "1.0.0",
...
{:ok, wf_math} = Workflows.create_workflow(scope, %{
...
{:ok, _} = Workflows.publish_workflow(scope, wf_math, %{
  version_tag: "1.0.0",
...
{:ok, _wf_aggregator} = Workflows.create_workflow(scope, %{
  ...
  connections: [
    %{id: SeedHelpers.conn_id(), source_node_id: agg_http_posts, target_node_id: agg_transform_posts},
    %{id: SeedHelpers.conn_id(), source_node_id: agg_http_users, target_node_id: agg_transform_users},
    %{id: SeedHelpers.conn_id(), source_node_id: agg_transform_posts, target_node_id: agg_merge},
    %{id: SeedHelpers.conn_id(), source_node_id: agg_transform_users, target_node_id: agg_merge},
...
{:ok, wf_webhook} = Workflows.create_workflow(scope, %{
  ...
  connections: [
    %{id: SeedHelpers.conn_id(), source_node_id: webhook_debug_in, target_node_id: webhook_transform},
    %{id: SeedHelpers.conn_id(), source_node_id: webhook_transform, target_node_id: webhook_format},
...
{:ok, _} = Workflows.publish_workflow(scope, wf_webhook, %{
  version_tag: "1.0.0",
...
{:ok, wf_legacy} = Workflows.create_workflow(scope, %{
...
{:ok, %{workflow: wf_legacy}} = Workflows.publish_workflow(scope, wf_legacy, %{
  version_tag: "1.0.0",
...
{:ok, wf_report} = Workflows.create_workflow(scope, %{
  ...
  connections: [
    ...
    %{id: SeedHelpers.conn_id(), source_node_id: report_transform, target_node_id: report_format},
...
{:ok, _} = Workflows.publish_workflow(scope, wf_report, %{
  version_tag: "1.0.0",
...
{:ok, _wf_empty} = Workflows.create_workflow(scope, %{
...
{:ok, _wf_demo} = Workflows.create_workflow(scope, %{
  ...
  nodes: [
    ...
    %{
      ...
      type_id: "transform",
      name: "Add Timestamp",
      config: %{"operation" => "set", "options" => %{"field" => "processed_at", "value" => "2024-01-01"}},
      position: %{"x" => 200, "y" => 250},
    ...
    %{
      ...
      type_id: "transform",
      name: "Rename Fields",
      config: %{"operation" => "rename", "options" => %{"mapping" => %{"name" => "fullName", "email" => "emailAddress"}}},
      position: %{"x" => 200, "y" => 350},
...
{:ok, _wf_error} = Workflows.create_workflow(scope, %{
...
IO.puts("\n📝 Created Workflows:")
for wf <- workflows do
  status_emoji = case wf.status do
    :draft -> "📝"
    :active -> "✅"
    :archived -> "📦"
  end
