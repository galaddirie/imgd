        ...
        <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="opacity-0 scale-95"
            ...
            <div v-if="show" ref="menuRef"
                ...
                <template v-for="(item, index) in items" :key="item.id">
                    <!-- Divider -->
                    <div v-if="item.divider" class="my-1.5 mx-2 border-t border-base-300" />

                    <!-- Menu Item -->
                    <button v-else class="w-full flex items-center gap-3 px-3 py-2 text-sm transition-colors" :class="[
                        item.disabled
                        ...
                        <span v-if="item.shortcut" class="text-xs text-base-content/40 font-mono">
                            {{ item.shortcut }}
                        </span>
                    </button>
                </template>
