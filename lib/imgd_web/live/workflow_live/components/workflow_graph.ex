defmodule ImgdWeb.WorkflowLive.Components.WorkflowGraph do
  ...
  defp graph_node(assigns) do
    ...
    <g class="workflow-node" data-node-id={@node.id}>
      ...
      <%= if @step_data do %>
        <% input_val = format_step_value(Map.get(@step_data, :input_data)) %>
        <% output_val = format_step_value(Map.get(@step_data, :output_data)) %>
        <% duration = Map.get(@step_data, :duration_ms) %>
  ...
  # Find step data for a given node ID (node_id)
  defp find_step_data(trace_steps, node_id) do
    Enum.find(trace_steps, fn step ->
      # Handle both struct (NodeExecution) and map (TraceStep)
      id = Map.get(step, :node_id) || step.node_id
      id == node_id
    end)
  end
  ...
  defp format_step_value(data) when is_map(data) do
    case Map.get(data, "value") || Map.get(data, :value) do
      nil -> "⋯"
      val when is_number(val) -> to_string(val)
      val when is_binary(val) and byte_size(val) <= 12 -> val
      _ -> "⋯"
    end
  end

  defp format_step_value(_), do: "⋯"
